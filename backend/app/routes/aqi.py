...
def generate_realistic_historical_data(location: dict, current_aqi: dict, state_name: str, days: int = 30):
    """Generate realistic historical AQI data based on current AQI and store in database"""
    try:
        now = datetime.datetime.now(datetime.timezone.utc)
        
        # Generate data for the past 'days' days (excluding today)
        for i in range(1, days + 1):
            ...
            existing = aqi_collection.find_one({
                ...
                "timestamp": {
                    ...
                    "$lt": (target_date + datetime.timedelta(days=1)).replace(hour=0, minute=0, second=0, microsecond=0)
                }
            })
            
            # Only generate if no data exists for this date
            if not existing:
                # Create realistic variations based on current AQI
                base_aqi = current_aqi["aqi"]
                if isinstance(base_aqi, str) and base_aqi.isdigit():
                    base_aqi = int(base_aqi)
                elif isinstance(base_aqi, str):
                    base_aqi = 50  # Default fallback value
                
                # Seasonal and weekly patterns
                day_of_week = target_date.weekday()
                month = target_date.month
                
                # Weekend effect (typically lower AQI on weekends)
                weekend_factor = 0.9 if day_of_week >= 5 else 1.0
                
                # Seasonal variations
                if month in [10, 11, 12, 1, 2]:  # Winter/post-monsoon - higher AQI
                    seasonal_factor = 1.2
                elif month in [3, 4, 5]:  # Summer - moderate AQI
                    seasonal_factor = 1.1
                else:  # Monsoon - lower AQI
                    seasonal_factor = 0.8
                
                # Generate realistic AQI with variations
                variation = random.uniform(-15, 15)  # ±15 AQI points
                realistic_aqi = max(0, int(base_aqi * weekend_factor * seasonal_factor + variation))
                
                # Generate other pollutants with realistic correlations
                def generate_pollutant(base_value, aqi_ratio=1.0, variation_range=0.2):
                    ...
                    return max(0, int(base_value * aqi_ratio * (realistic_aqi / base_aqi) * (1 + variation)))
                
                historical_data = {
                    "aqi": realistic_aqi,
                    "city": current_aqi["city"],
                    "dominant": current_aqi.get("dominant", "N/A"),
                    "pm25": generate_pollutant(current_aqi.get("pm25", 0), 0.8),
                    "pm10": generate_pollutant(current_aqi.get("pm10", 0), 1.2),
                    "o3": generate_pollutant(current_aqi.get("o3", 30), 0.6),
                    "co": generate_pollutant(current_aqi.get("co", 1), 0.5),
                    "no2": generate_pollutant(current_aqi.get("no2", 20), 0.7),
                    "so2": generate_pollutant(current_aqi.get("so2", 5), 0.4),
                    "lat": location["lat"],
                    "lon": location["lon"],
                    "state": state_name,
                ...
                safe_insert(aqi_collection, historical_data)
                logger.info(f"Generated historical data for {target_date.strftime('%Y-%m-%d')} - AQI: {realistic_aqi}")
        
        logger.info(f"Historical data generation completed for {days} days")
        return True
